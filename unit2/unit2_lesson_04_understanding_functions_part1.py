...
def get_min_max_words(input):
    ...
    """
    input = list(input)
    return input[find_len(input).index(min(find_len(input)))], input[find_len(input).index(max(find_len(input)))]
...
-every function has a return object
-one function can be passed as a argument to other function
"""